...
| publish-open-vsx.yml | publish (matrix) | `open_vsx_publish` | Submodules `compiler`, `beskid_vscode` via Nox `ci/submodules.py`. Matrix: `linux-x64`, `darwin-arm64`, `darwin-x64`, `win32-x64`. Env: `COMPILER_SUBMODULE_TOKEN`, optional `BESKID_VSCODE_SUBMODULE_TOKEN` / `BESKID_VSCODE_SUBMODULE_URL`, `OPENVSX_PLATFORM`, `OPENVSX_BIN_NAME`, optional `OPENVSX_RUST_TARGET` (e.g. `x86_64-apple-darwin` for Intel mac LSP on Apple runners), `OVSX_TOKEN` |
