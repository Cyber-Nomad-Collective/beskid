...
secret_setup="${root}/scripts/ci/set-distrib-secrets.sh"
release_checklist="${root}/docs/orchestrate/verify-all-on-main-v04.md"
download_api="${root}/site/website/src/pages/api/version.json.ts"
candidate_downloads="${root}/beskid_sites/apps/website/src/content/book/01-it-works-on-my-machine/downloads-and-releases.md"
...
grep -Fq 'needs: [resolve-rolling, windows-msi, macos-brew, macos-dmg, ubuntu-deb, container-images]' "${workflow}" || fail "completion marker must wait for every supported platform job"
...
if grep -Eiq 'linux-snap|snapcraft|canonical/action-(build|publish)|SNAPCRAFT_STORE_CREDENTIALS' "${workflow}"; then
  fail "retired Snap distribution must not remain in distribute.yml or block completion"
fi
if grep -Eiq 'snap store|snapcraft|snap install beskid|linux-snap|SNAPCRAFT_STORE_CREDENTIALS|label: "Snap"' \
  "${secret_setup}" "${release_checklist}" "${download_api}" "${candidate_downloads}"; then
  fail "retired Snap distribution remains on a current operator or download surface"
fi
...
# dpkg-deb runs.
ubuntu_block="$(sed -n '/^  ubuntu-deb:/,/^  container-images:/p' "${workflow}")"
