...
assert_contains "${gate_src}" 'pnpm --dir="${NEXUS_WEB}" test:unit' "gate invokes Nexus unit runner"
assert_contains "${gate_src}" 'pnpm --dir="${NEXUS_WEB}" test:e2e' "gate invokes Nexus Playwright E2E"
assert_contains "${gate_src}" 'test:e2e:install' "gate installs Playwright Chromium"
# Reject Bun invocations: the gate is pnpm-only across all packages.
if rg -n '(^|[[:space:]])bun([[:space:]]|$)' "${gate}" >/dev/null 2>&1; then
  echo "  FAIL - gate must not invoke bun (pnpm-only contract)" >&2
  exit 1
fi
_TESTS_RUN=$((_TESTS_RUN + 1))
