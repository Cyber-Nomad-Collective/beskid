...
doc="${root}/docs/release/shared-ui-nexus-gate.md"
...
gate_src="$(cat "${gate}")"
assert_contains "${gate_src}" 'bun --cwd "${WEB_COMMON}" test' "gate invokes shared UI package test"
assert_contains "${gate_src}" 'run test:unit' "gate invokes Nexus unit runner"
assert_contains "${gate_src}" 'run test:e2e' "gate invokes Nexus Playwright E2E"
