...
require 'REGISTRY_USERNAME' "$env_example"
require 'REGISTRY_PASSWORD' "$env_example"
require 'need REGISTRY_USERNAME' "$deploy_script"
require 'need REGISTRY_PASSWORD' "$deploy_script"
...
[[ "${pckg_block}" == *"SHELL_AUTH_MODE: authentik"* ]] || {
  echo 'pckg must use Authentik mode while parsing the identity injected by the edge' >&2
  exit 1
}
