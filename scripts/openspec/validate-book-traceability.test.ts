...
const capabilityPath = "/platform-spec/capabilities/language-meta--surface-syntax--documentation-comments/";
...
function withBook(files: Record<string, string>, run: (bookRoot: string) => void): void {
...
test("accepts canonical standard links and derives sorted reciprocal Book routes", () => {
	withBook({
		...
		const result = validateBookTraceability({
			catalog: catalogFor([
				{
					path: "site/website/src/content/docs/book/20-doc-comments-that-are-not-lies/z-guide.md",
					standardLinks: [capabilityPath],
				},
				{
					path: "site/website/src/content/docs/book/20-doc-comments-that-are-not-lies/a-guide.md",
					standardLinks: [capabilityPath],
				},
			]),
			bookRoot,
...
test("derives a public Book route from a catalog document path", () => {
	withBook({ "13-reading-the-law/platform-spec-home.md": "# Platform spec home\n" }, (bookRoot) => {
		const result = validateBookTraceability({
			catalog: catalogFor([
				{
					path: "site/website/src/content/docs/book/13-reading-the-law/platform-spec-home.md",
					standardLinks: [capabilityPath],
				},
			]),
			bookRoot,
		...
		assert.deepEqual(result.bookLinksByCapability["language-meta--surface-syntax--documentation-comments"], [
...
test("reports an uncovered technical Book document", () => {
	withBook({ "20-doc-comments-that-are-not-lies/technical.md": "# Technical guide\n" }, (bookRoot) => {
		const result = validateBookTraceability({
			catalog: catalogFor([
				{
					path: "site/website/src/content/docs/book/20-doc-comments-that-are-not-lies/technical.md",
					standardLinks: [],
				},
			]),
			bookRoot,
...
test("allows a Book document explicitly declared as narrative", () => {
	withBook({
		...
		const result = validateBookTraceability({
			catalog: catalogFor([
				{
					path: "site/website/src/content/docs/book/20-doc-comments-that-are-not-lies/narrative.md",
					standardLinks: [],
				},
			]),
			bookRoot,
