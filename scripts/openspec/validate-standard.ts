...
assert(
	...
	"OpenSpec is not declared as the sole authority",
);
...
assert(
	...
	"Duplicate archived artifact paths",
);
assert(
	!requirements.some((item) => item.migrationStatus === "needs-semantic-review"),
	"Unreviewed migration requirements remain",
);
assert(
	specDocuments.length > 0,
	"Canonical Platform Spec document catalog is empty",
);
...
for (const document of specDocuments) {
	...
	assert(
		...
		`Missing canonical document ${canonicalPath}`,
	);
	assert(
...
for (const entry of entries) {
	...
	for (const record of (entry.records as UnknownRecord[]) ?? []) {
		assert(
			aliases.has(String(record.legacySlug)),
			`Missing legacy alias ${record.legacySlug}`,
		);
	}
}
...
for (const document of documents) {
	...
	assert(
		document.authority === "informative",
		`Document is not informative: ${document.path}`,
	);
	assert(
		document.disposition === "informative-by-policy",
		`Document lacks authority disposition: ${document.path}`,
	);
	assert(
		document.sourceHash === sha256(readFileSync(documentPath, "utf8")),
		`Document catalog drift: ${document.path}`,
	);
}
...
for (const entry of entries) {
	...
	assert(
		JSON.stringify(actualBookLinks) ===
			JSON.stringify(expectedBookLinks[capability] ?? []),
...
for (const asset of [
	...
	assert(
		existsSync(path.join(repoRoot, "openspec/assets/architecture", asset)),
		`Missing architecture asset ${asset}`,
	);
}

console.log(
	`Canonical Beskid standard valid: ${entries.length} capabilities, ${requirements.length} requirements, ${records.length} archived source records, ${artifacts.length} archived artifact hashes.`,
);
