...
- Add a deterministic, advisory STE review command for technical Docs. It
  ...
  expressions; evaluates abbreviation explanations in document order; joins
  lazy list continuations; detects controlled regular and irregular passive
  forms; skips symbolic links and syntax that must stay exact; and rejects vague,
...
- Replace Learn's flat workspace columns with a persisted, validated mosaic layout
  that preserves learner-selected split sizes and supports nested pointer and
  keyboard resizing.
...
- Emit one structured CI diagnostic per failed Rust test with its matching
  panic location and nearby reason instead of pairing a test name with an unrelated log tail.
...
- Keep Windows runtime provenance export checks exact while classifying MSVC
  string-literal COMDATs and the COFF feature marker as compiler metadata.
