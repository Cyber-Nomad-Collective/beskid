...
There is no `/platform-spec/<version>/...` tree. The contract at commit **`abc123`** is whatever normative text and metadata exist at **`abc123`**.
...
- A behavioral change edits the page body, `lastReviewed`, ADR status, and embedded decisions. It does not touch the URL.
