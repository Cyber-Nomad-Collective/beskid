...
describe("Platform Spec shared UI styles", () => {
	it("loads the shared component source, sidebar tokens, and hub presentation", () => {
		...
		expect(source).toContain(
			'@source "../node_modules/@cyber-nomad-collective/beskid-ui-react/src"',
		);
		expect(source).toContain(
			'@import "@cyber-nomad-collective/beskid-ui-react/styles/shadcn-entry.css"',
		);
		expect(source).toContain(
			'@import "@cyber-nomad-collective/beskid-ui-react/styles/hub.css"',
		);
	});
	...
	it("verifies the emitted stylesheet during production builds", () => {
		...
		expect(packageJson).toMatch(/"build":\s*"[^"]*pnpm run verify:client-bundle/);
	});
});
