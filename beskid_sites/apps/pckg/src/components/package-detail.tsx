...
export function PackageDetail({
	...
	const presentation = packageKindPresentation({
		...
		templateShortName: pkg.template?.shortName,
	});
	return (
				...
				<div className="flex flex-wrap gap-2">
					...
					{presentation.showDocumentation && (
						<Link
							...
							search={{ version: details.latestVersion ?? "", doc: "", source: "" }}
				...
				<Card className="mt-6">
					...
					<CardContent>
						<p className="text-sm text-muted-foreground">
							{presentation.instructions.description}
						</p>
						{presentation.instructions.commands.length > 0 && (
							<pre className="mt-3 overflow-x-auto rounded-md border border-border p-3 text-sm">
								{presentation.instructions.commands.join("\n")}
							</pre>
						)}
					</CardContent>
				</Card>
