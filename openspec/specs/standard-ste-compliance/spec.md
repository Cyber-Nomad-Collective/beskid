# standard-ste-compliance Specification

## Purpose
This specification adopts ASD-STE100 Simplified Technical English as the controlled language for the OpenSpec prose. ASD-STE100 is Issue 9, January 2025. The standard defines a set of writing rules and a controlled dictionary. Each approved word has one meaning and one part of speech. The standard permits technical nouns and verbs for subject-specific terms. This specification defines the scope of the STE rules, the exempt sections, the permitted technical vocabulary, and the conformance checks.
...
Requirement statements in OpenSpec specs SHALL use the `SHALL` or `MUST` keyword with a simple sentence structure. Requirement statements are exempt from the ASD-STE100 controlled dictionary. Requirement statements SHALL NOT be rewritten as descriptive prose.
...
Scenario statements in OpenSpec specs SHALL keep the `GIVEN`/`WHEN`/`THEN` form with optional `AND` bullets. Scenario statements are exempt from the ASD-STE100 controlled dictionary.
...
The ASD-STE100 controlled dictionary SHALL permit Beskid technical nouns and verbs as subject-specific terms. The permitted terms include at minimum: fiber, spawn, lowering, codegen, corelib, runtime, ABI, CLIF, ISLE, pest, salsa, abfall, Cranelift, HIR, AST, LSP, JIT, AOT, GC, and Mod. Additional Beskid-specific terms are permitted when a spec defines them.
