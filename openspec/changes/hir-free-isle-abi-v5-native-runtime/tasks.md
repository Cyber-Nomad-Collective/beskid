...
- [ ] 6.8.1c Implement the canonical scheduler from ABI-owned state and target-derived contexts/stacks; prove syntax-to-ISLE spawn/yield/join behavior, Phase-A safepoints, and main-fiber shutdown before claiming a supported target.
  ...
  - [ ] 6.8.1c.3 Implement the one canonical lifecycle for fiber 0 plus spawn, cancel-slot spawn, yield/resume, cancel, detach, current-id, join-status, and join-value; prove status/value truth and one scheduler owner through syntax-to-ISLE verified CLIF.
...
- [ ] 6.8.1f Replace the canonical Process environment, filesystem, and terminal placeholder exports with manifest-owned target adapters and fail-closed normalized outcomes.
  - [ ] 6.8.1f.0 Add a manifest-owned intrinsic-to-target adapter-binding model, including empty-string versus unavailable scratch-ownership policy, before declaring Process imports or native implementations.
