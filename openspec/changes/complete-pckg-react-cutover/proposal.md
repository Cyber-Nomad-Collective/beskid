...
The registry package builds a React/Vite client but the running server still
maps its root route through the legacy interactive Blazor host. The served
...
- Verify that the React entrypoint and a nested client route are served while
  server-owned paths retain their non-SPA behavior.

## Impact

- `pckg/web`, `pckg/src/Server`, and browser/server conformance tests.
