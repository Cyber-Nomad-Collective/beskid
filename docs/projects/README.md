...
- `docs/projects/build-workflow.md` for resolve/materialize/build lifecycle
- `docs/projects/lockfile.md` for `Project.lock` behavior
...
3. Keep compatibility fallback behind an explicit feature flag for one release window, then remove.
4. Disable fallback injection whenever a resolvable `Std` dependency node exists in the project DAG.
