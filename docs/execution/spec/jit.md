...
## Purpose
The JIT is the primary execution engine. The AOT path should reuse the same frontend and module logic, only swapping the backend implementation.
...
3. Declare all function signatures with `declare_function`.
...
## JIT as backbone for AOT
- The frontend emits CLIF once.
- The module abstraction is shared.
