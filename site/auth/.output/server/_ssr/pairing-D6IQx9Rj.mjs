...
//#region node_modules/.nitro/vite/services/ssr/assets/pairing-D6IQx9Rj.js
var pairing_D6IQx9Rj_exports = /* @__PURE__ */ __exportAll$1({
	A: () => AUTH_APP_META,
	C: () => resolveOAuthConfig,
	D: () => getServiceTokenForApp,
	E: () => getPairedApp,
	M: () => HUB_USER_TOKEN_TTL_SECONDS,
	O: () => listEnabledApps,
	S: () => readAuthConfig,
	T: () => writeAuthConfig,
	_: () => removeAdminLogin,
	a: () => listPairingAudit,
	b: () => isOAuthConfigured,
	c: () => clearSessionCookieHeader,
	d: () => sealHubBrowserSession,
	f: () => createUserSession,
	g: () => promoteBootstrapAdminIfNeeded,
	h: () => getAdminLogins,
	i: () => getPairingRequest,
	j: () => AUTH_HUB_ISSUER,
	k: () => env,
	l: () => getSessionFromRequest,
	m: () => addAdminLogin,
	n: () => cancelPairingRequest,
	o: () => listPairingRequests,
	p: () => getGithubTokenForSession,
	r: () => createPairingRequest,
	s: () => pairing_exports,
	t: () => approvePairing,
	u: () => hubBrowserSessionCookieHeader,
	v: () => getAppById,
	w: () => verifySetupToken,
	x: () => isOnboarded,
	y: () => isAdminLogin
...
var pairing_exports = /* @__PURE__ */ __exportAll({
	approvePairing: () => approvePairing,
	cancelPairingRequest: () => cancelPairingRequest,
	createPairingRequest: () => createPairingRequest,
	getPairingRequest: () => getPairingRequest,
	listPairingAudit: () => listPairingAudit,
	listPairingRequests: () => listPairingRequests
});
...
function getPairingRequest(id) {
	return getAuthDatabase().query("SELECT * FROM pairing_requests WHERE id = ?").get(id) ?? null;
}
function listPairingAudit(requestId) {
	return getAuthDatabase().query("SELECT * FROM pairing_audit WHERE request_id = ? ORDER BY created_at ASC").all(requestId);
}
function cancelPairingRequest(id, actorLogin) {
	const row = getPairingRequest(id);
	if (!row) return { error: "Pairing request not found" };
	if (row.status !== "pending") return { error: "Only pending pairing requests can be cancelled" };
	getAuthDatabase().prepare("UPDATE pairing_requests SET status = 'cancelled' WHERE id = ?").run(id);
	audit(id, "cancelled", actorLogin);
	return { ok: true };
}
//#endregion
export { sealHubBrowserSession as A, listPairingAudit as C, readAuthConfig as D, promoteBootstrapAdminIfNeeded as E, writeAuthConfig as M, removeAdminLogin as O, listEnabledApps as S, pairing_D6IQx9Rj_exports as T, getSessionFromRequest as _, approvePairing as a, isOAuthConfigured as b, createPairingRequest as c, getAdminLogins as d, getAppById as f, getServiceTokenForApp as g, getPairingRequest as h, addAdminLogin as i, verifySetupToken as j, resolveOAuthConfig as k, createUserSession as l, getPairedApp as m, AUTH_HUB_ISSUER as n, cancelPairingRequest as o, getGithubTokenForSession as p, HUB_USER_TOKEN_TTL_SECONDS as r, clearSessionCookieHeader as s, AUTH_APP_META as t, env as u, hubBrowserSessionCookieHeader as v, listPairingRequests as w, isOnboarded as x, isAdminLogin as y };
