...
if rg -l 'SESSION_SECRET|AUTH_HUB_SECRET|GITHUB_CLIENT_SECRET|env\.server' "${assets}"/*.js >/dev/null 2>&1; then
	echo "verify-client-bundle: forbidden symbols found in client assets:" >&2
	rg -n 'SESSION_SECRET|AUTH_HUB_SECRET|GITHUB_CLIENT_SECRET|env\.server' "${assets}"/*.js >&2 || true
	exit 1
fi
