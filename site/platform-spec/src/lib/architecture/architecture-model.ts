...
export interface ArchitectureManifest {
	...
	traversals: Readonly<
		Record<"build" | "ide" | "spec-to-code", readonly string[]>
	>;
}
...
function resolveSpecLinks(
	...
	return node.specKeys.map((capability) => {
		...
		if (node.state === "current") {
			throw new Error(
				`unresolved current spec key "${capability}" on node "${node.id}"`,
			);
		}
...
export function resolveArchitectureModel(
	...
	const traversals = manifest.traversals as Readonly<
	...
	const catalogByCapability = new Map(
		catalogEntries.map((entry) => [entry.capability, entry]),
	);
	...
	for (const node of manifest.nodes) {
		...
		if (!groupIds.has(node.group))
			throw new Error(`unknown group "${node.group}" on node "${node.id}"`);
		nodesById[node.id] = {
			...node,
			specLinks: resolveSpecLinks(node, catalogByCapability),
		};
	}
	...
	for (const [name, traversal] of Object.entries(manifest.traversals)) {
		for (const nodeId of traversal) {
			if (!nodesById[nodeId])
				throw new Error(`traversal "${name}" has unknown node "${nodeId}"`);
		}
	}
