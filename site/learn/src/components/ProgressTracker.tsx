...
import { Card, CardHeader, CardTitle, CardDescription, CardContent, Badge } from '@beskid/ui-react';
...
function ProgressTracker({ passedLessons, exerciseCount, categories, streak = 0 }: ProgressTrackerProps) {
  ...
  const achievements = useMemo(() => {
    const result: Array<{ id: string; label: string; description: string; earned: boolean }> = [];
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {Object.entries(categories).map(([key, cat]) => {
            const catPct = cat.count > 0 ? Math.round((cat.completed / cat.count) * 100) : 0;
            return (
              <div key={key}>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {achievements.map((ach) => (
              ...
              >
                <Trophy className={`mr-1 size-3 ${ach.earned ? 'text-amber-500' : ''}`} />
                {ach.label}
              </Badge>
            ))}
          </div>
