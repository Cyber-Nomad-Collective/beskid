...
- Its compiler stage copies the canonical `compiler/scripts` runtime-kit staging entrypoint and
  builds `beskid_cli` plus the ABI-v5 kit under one explicit Cargo target prefix; the final image
  contains only that CLI and installed kit, not the compiler source tree.
