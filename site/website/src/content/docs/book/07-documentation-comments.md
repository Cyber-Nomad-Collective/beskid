...
Documentation lives next to declarations in **`///` line comments** only. A doc run can be attached to top-level declarations and nested members (fields, variants, contract members, methods, parameters, and test-body statements). Write Markdown in the body, and use **`@arg`**, **`@returns`**, **`@variant`**, **`@par`**, and **`@ref`** for structured sections.
