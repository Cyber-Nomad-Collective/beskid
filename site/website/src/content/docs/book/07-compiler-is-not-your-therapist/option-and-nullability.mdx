...
Optional presence **must** use `Option<T>` (corelib `Core.Optional.Option`) or an explicit `enum` with a dedicated absent variant at API boundaries.
...
```mermaid
classDiagram
  accTitle: Option as an enumeration
  accDescr: Option of T is an enumeration with the variants Some and None, and a match on it handles both.
  class Option~T~ {
    <<enumeration>>
    Some(T)
    None
  ...
  class MatchExpression {
    handles None as absent
    handles Some as inner T
  }
  MatchExpression ..> Option : narrows
```

**Text equivalent:** `Option<T>` is an enumeration with variants `Some(T)` and `None`. A `match` narrows it, handling the absent case and using the inner `T` in the present case.
