...
description: Normative API tiering for corelib modules — three stability tiers, prelude rules, and the `api.json` contract that surfaces them to consumers.
template: splash
...
relatedTopics:
  ...
  - type: Feature
    ...
    href: /platform-spec/core-library/compiler-integration/documentation-pipeline/
    relation: api.json is the transport for tier metadata
---
...
- Tier-tagged corelib sources: `compiler/corelib/packages/foundation/src/Collections/`, `compiler/corelib/packages/runtime/src/System/`
