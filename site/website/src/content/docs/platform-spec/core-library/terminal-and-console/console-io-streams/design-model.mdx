...
relatedTopics:
  ...
  - type: Feature
    ...
    relation: Builtin read/write implementation
---
...
- Implementation: `compiler/corelib/packages/runtime/src/System/Input/Input.bd`, `Output/Output.bd`, `Error/Error.bd`, `Syscall/`
...
User code **must not** pass integer fds to `ReadWith` / `WriteWith` for these helpers. Arbitrary descriptors, sockets, and pipes are out of scope for v1 stream modules; binary and raw-fd I/O lives in [System.Syscall](/platform-spec/core-library/stability-and-api-shape/system-syscall/) (**SC-002**, **SC-003**), which is a superset profile of which console streams are the **SC-004** restricted subset.
