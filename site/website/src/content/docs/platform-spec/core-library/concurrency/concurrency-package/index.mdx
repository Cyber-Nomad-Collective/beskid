...
1. **`spawn entry()`** lowers to runtime spawn with stack map for `entry`; parent gets `Fiber<T>`.
...
- **Detach** child panic → process abort after diagnostic (Go-like default) unless runtime policy adds domain recovery later.
...
- Runtime integration: `compiler/crates/beskid_tests/src/runtime/fibers.rs` (to be added)
- Conformance: rendezvous **Send**/**Receive**, **Cancel**, **Hub** readiness ordering
</SpecSection>

<SpecSection title="Related features" id="related-features">
- **[Decisions record](./decisions-record/)** — closed pre-implementation decisions (normative)
