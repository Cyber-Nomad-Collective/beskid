...
relatedTopics:
  ...
  - type: Feature
    ...
    relation: Template scaffolding entrypoint
---
...
1. The CLI shall expose stable command families for `run`, `build`, `test`, `repl`, `analyze`, `parse`, `format`, `clif`, `doc`, `corelib`, `lock`, `fetch`, `tree`, `update`, and **`new`** (project, workspace, and item templates).
...
- `compiler/crates/beskid_tools` — shared pipeline UI, diagnostics, session, registry helpers
