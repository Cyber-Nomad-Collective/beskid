...
`{version}` may be `latest` or an exact registry version.
...
5. When **`declaringPackage`** is present and differs from the page package, group under **Dependencies → {declaringPackage}** before module nesting.
