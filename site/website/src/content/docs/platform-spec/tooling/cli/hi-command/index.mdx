---
title: Hi command
description: Pluggable terminal dashboard shell with BSOL board layouts and a command palette.
template: splash
...
relatedTopics:
  ...
  - type: Feature
    ...
    relation: Pipeline commands open compile widget instead
---
...
The hamburger menu (`m`) shows a hierarchical tree: built-in compiler tools (graphs, compile debugger, analysis, settings, debugger reserved), project flows, user boards, plus entries merged from extension `NavRegistrar` hooks.
