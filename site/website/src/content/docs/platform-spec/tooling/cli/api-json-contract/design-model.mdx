...
| Multi-file projects | `beskid doc --project` uses **workspace scan** assembly (all `*.bd` under host + path dependency roots), merges resolution per unit, and sets `location.file` per declaring unit |
...
| `source` | yes | Entry source path used for the doc run |
...
| `declaringPackage` | Workspace scan / aggregate publish | Registry package id when `location.file` is outside the publishing project root (optional on v4; omit when same package) |
