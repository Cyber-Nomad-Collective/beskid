...
When stderr is a TTY and `--plain` is not set, pipeline-backed commands render phase progress on the alternate screen, then show a **summary screen** (stats table and optional pie chart for tests or analyze severities) before returning to the shell. The same facts are always echoed as a one-line stderr summary after the TUI exits so scripts and CI logs stay parseable. `--plain` skips the alternate-screen UI entirely.
