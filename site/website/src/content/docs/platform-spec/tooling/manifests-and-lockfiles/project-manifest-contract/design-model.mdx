...
Tooling-facing shape of **`Project.proj`**: project kind, mod orchestration block, template metadata, readme publishing, and host `app` launch rules. Resolution-graph behavior and diagnostic bands live in **[Project manifest contract (compiler)](/platform-spec/compiler/resolution-and-projects/project-manifest-contract/)**—this article does not duplicate full manifest key tables from the manifests area hub.
