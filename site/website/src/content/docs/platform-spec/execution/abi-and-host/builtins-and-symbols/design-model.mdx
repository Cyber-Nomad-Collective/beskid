...
- `compiler/crates/beskid_abi/src/builtins.rs` — `BuiltinFnSpec` entries and `BUILTIN_SPECS` table
- `compiler/crates/beskid_abi/src/symbols.rs` — `RUNTIME_EXPORT_SYMBOLS` and `SYM_*` constants
- `compiler/crates/beskid_runtime/src/builtins/` — `#[unsafe(no_mangle)]` runtime implementations
