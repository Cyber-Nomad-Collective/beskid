...
| **B (documented)** | Multiple parallel mutators | Go-style concurrent mark/sweep + real **gc_write_barrier** | Optional function-entry preemption |
...
- `fiber_spawn`, `fiber_join`, `fiber_detach`, `fiber_cancel`, `fiber_yield`
- `fiber_current_id` (diagnostics)
...
- `compiler/crates/beskid_runtime/` — scheduler module (to be added)
