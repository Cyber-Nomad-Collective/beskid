...
host InfraHost : ConsoleHost {
    registry {
        single SharedConfig for IConfiguration;
    }
}

host AppHost(string[] args) : InfraHost {
    registry {
        single AppConfig for IConfiguration;   // overrides InfraHost / ConsoleHost for IConfiguration
...
**Override key:** contract type for `for IContract` lines; implementation type for self-bound lines.
...
host AppHost(string[] args) : ConsoleHost {
    ...
    scope HttpScope(RequestContext request) {
        ...
        init(
            global::IConfiguration configuration,
            AuthService auth,
        ) {
            // per activation
        }

        dispose(IDbSession db, AuthService auth) {
            auth.SignOut();
            db.Close();
        }

        scope UnitOfWork(bool readOnly) {
            Transaction for ITransaction;

            init(ITransaction tx) {
                tx.Begin(readOnly);
            }

            dispose(ITransaction tx) {
                tx.Commit();
            }
        }
    }
...
```beskid
single Implementation for IContract;
...
type Aggregator {
    ...
    inject IStorage[] storages;              // all IStorage registrations in resolution container
    inject global::ILogger logger;         // force global registry
}
```
