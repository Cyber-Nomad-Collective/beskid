...
Apps register multiple implementations of one contract (for example two `IStorage`).

## Decision

Multiple implementations **must** use **`inject IContract[]`** (or concrete **`T[]`**). Singular **`inject IContract`** **must** be unique at the resolution level (**E1705** when ambiguous).
