---
title: Types
description: The type grammar (nominal types, generics, ref T, Option T) is
  the backbone of static checking. All analysis phases share these definitions.
template: splash
...
- **Fields** **may** be value fields (`T name`), `event` fields (see [Events](/platform-spec/language-meta/evaluation/events/)), or `inject` fields (composition only).
...
- `extend type` adds members externally per [extend type](/platform-spec/language-meta/program-structure/extend-type/).
