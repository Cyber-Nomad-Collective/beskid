...
relatedTopics:
  ...
  - type: Area
    ...
    relation: Meta diagnostics bands and Mod SDK parity
---
...
Codes **E1801–E1899** are owned by manifest, workspace, meta host, emit merge, and `meta` surface validation—not by ordinary semantic rules. When adding meta-related failures, update **[Diagnostic code registry / design model](/platform-spec/compiler/semantic-pipeline/diagnostic-code-registry/)** first, then reference the new code from rule modules or manifest validators so the catalog remains single-sourced.
