...
When the **Rust compiler** maintains compile-time **composition / IoC** state for user programs, that state **must** be materialized into the same **versioned semantic snapshot** family as other staged products so mod analyzers can query it without special casing: Mod SDK query types **project** compiler-native graphs into Beskid-visible handles; they **must not** recompute or replace Rust-owned resolution unless a dedicated, separately specified capability allows a narrow, audited code path.
