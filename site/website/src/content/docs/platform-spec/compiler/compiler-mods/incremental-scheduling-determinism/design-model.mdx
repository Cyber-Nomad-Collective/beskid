...
Within one **`meta.round_commit`** event, emit applications and diagnostic emissions **must** commit atomically: downstream semantic stages **must not** observe partially applied contributor edits (**[Typed emitter and transforms](/platform-spec/compiler/compiler-mods/typed-emitter-and-transforms/)**).
