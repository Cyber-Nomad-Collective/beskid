---
title: AST and HIR shape contract - Design model
description: Explains the persistent concepts, entities, and boundaries this feature relies on.
template: splash
tableOfContents: true
specLevel: article
status: Standard
owner:
  name: Piotr Mikstacki
  email: pmikstacki@cybernomad.it
submitter:
  name: Piotr Mikstacki
  email: pmikstacki@cybernomad.it
relatedTopics:
  - type: Feature
    title: AST and HIR shape contract
    href: /platform-spec/compiler/front-end/ast-hir-shape-contract/
    relation: Parent feature hub
---

import SpecArticleChrome from '@beskid/docs-ui/platform-spec/SpecArticleChrome.astro';

<SpecArticleChrome />



This article documents **design model** for **ast and hir shape contract** in the reference compiler.

## What this covers
For newcomers, this page explains where the contract shows up in day-to-day compiler work and which code paths are most useful first reads.

## Anchored code paths
- `compiler/crates/beskid_analysis/src/syntax/items/` exposes AST-like syntax node shapes.
- `compiler/crates/beskid_analysis/src/resolve/items.rs` maps parsed items into resolved structures.
- `compiler/crates/beskid_analysis/src/analysis/` consumes those shapes for semantic passes.

## `meta` items and HIR

Once `meta` exists in the syntax tree, lowering **must** either (a) map `meta` items to dedicated HIR metadata nodes consumed only by the meta host, or (b) strip them after host registration while preserving spans for diagnostics—pick one strategy, document it here, and keep it consistent with **[Rules pipeline contract](/platform-spec/compiler/semantic-pipeline/rules-pipeline-contract/)** and **[Typed emitter and transforms](/platform-spec/compiler/metaprogramming-mod-sdk/typed-emitter-and-transforms/)**. HIR **must not** partially encode unmerged emit contributions.

## Practical notes
- Prefer tracing from CLI/test entry points into analysis/codegen crates before changing internals.
- Treat diagnostics and tests as part of the contract, not optional implementation details.
- If behavior changes, update this article and add/adjust tests in `compiler/crates/beskid_tests` or `compiler/crates/beskid_e2e_tests`.
