...
## 5.4 Current support boundaries

The features listed below should be treated as unsupported in the current transpiler contract.

- `record`-specific syntax/semantics beyond plain `type` mapping.
- Iterator sugar based on `yield return`.
- Checked arithmetic semantics (`checked`/`unchecked`) as C# defines them.
- `decimal` numeric semantics and fixed-point precision guarantees.
- `async`/`await` task-based execution model.
- Synchronization constructs such as `lock`, `SemaphoreSlim`, and `Mutex`.
- Operator-overload declarations from C# source.
- Stack-scoped memory abstractions (`Span<T>`, `Memory<T>`, `ref struct`).
- Runtime reflection and expression-tree APIs.

Any feature graduating from unsupported to supported must be documented by:

1. removing or changing its unsupported error entry in this document,
2. defining exact lowering semantics in the compatibility spec,
3. adding conformance tests for the supported behavior.
