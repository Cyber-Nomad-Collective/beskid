use pecan_analysis::syntax::{Expression, Literal, Pattern};
...
fn parses_binary_expression_ast() {
    ...
    match &expr.node {
        Expression::Binary(binary) => {
            assert!(matches!(binary.node.op.node, pecan_analysis::syntax::BinaryOp::Add));
            ...
            match &binary.node.right.node {
                Expression::Binary(right_binary) => {
                    assert!(matches!(
                        ...
                        pecan_analysis::syntax::BinaryOp::Mul
                    ));
        ...
        Expression::Match(match_expr) => {
            ...
            match first.node.guard.as_ref() {
                Some(guard) => match &guard.node {
                    Expression::Binary(binary) => {
                        assert!(matches!(binary.node.op.node, pecan_analysis::syntax::BinaryOp::Gt));
...
fn parses_unary_expression_ast() {
    ...
    match &expr.node {
        Expression::Unary(unary) => {
            assert!(matches!(unary.node.op.node, pecan_analysis::syntax::UnaryOp::Neg));
