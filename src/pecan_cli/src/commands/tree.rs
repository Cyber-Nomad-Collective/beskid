...
use pecan_analysis::query::NodeRef;
use pest::Parser;
use crate::errors::{print_pretty_parse_error, print_pretty_pest_error};

#[derive(Args, Debug)]
pub struct TreeArgs {
    /// The input Pecan file to visualize
    #[arg(required = true)]
    pub input: PathBuf,
}
...
fn print_tree(node: NodeRef, indent: usize) {
    ...
    let extra = if let Some(ident) = node.of::<pecan_analysis::syntax::Identifier>() {
        format!(" ({})", ident.name)
    } else if let Some(lit) = node.of::<pecan_analysis::syntax::Literal>() {
        format!(" ({:?})", lit)
    } else {
        String::new()
    };
