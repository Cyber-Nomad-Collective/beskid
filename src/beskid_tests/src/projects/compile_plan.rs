...
"#;
    ...
    let plan = build_compile_plan(&manifest_path, None, true).expect("plan should build");
    assert_eq!(plan.project_name, "MyApp");
    assert_eq!(plan.target.name, "App");
    assert_eq!(plan.target.kind, TargetKind::App);
    assert!(plan.allow_stdlib_prelude_fallback);
...
"#;
    ...
    let plan = build_compile_plan(&manifest_path, Some("Library"), false).expect("plan should build");
...
"#;
    ...
    let error = build_compile_plan(&manifest_path, Some("Tests"), false).expect_err("must fail");
...
"#;
    ...
    let plan = build_compile_plan(&app_manifest_path, None, true).expect("plan should build");
...
"#,
    ...
    let plan = build_compile_plan(&app_manifest_path, None, false).expect("plan should build");
...
"#;
    ...
    let error = build_compile_plan(&app_manifest_path, None, false).expect_err("must fail");
...
#[test]
fn compile_plan_disables_prelude_fallback_when_std_dependency_exists() {
