...
impl<'a> TypeContext<'a> {
    pub(super) fn type_id_for_type(&mut self, ty: &Spanned<Type>) -> Option<TypeId> {
        match &ty.node {
            ...
            Type::Complex(path) => self.type_id_for_type_path(path.span),
            Type::Array(inner) | Type::Ref(inner) => self.type_id_for_type(inner),
        }
    }

    pub(super) fn type_id_for_type_path(&mut self, span: crate::syntax::SpanInfo) -> Option<TypeId> {
