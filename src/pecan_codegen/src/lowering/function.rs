...
use pecan_analysis::resolve::{ItemId, LocalId, Resolution};
use pecan_analysis::syntax::Spanned;
use pecan_analysis::types::{TypeInfo, TypeResult};
...
pub(crate) fn mangle_function_name(base: &str, args: &[pecan_analysis::types::TypeId]) -> String {
    if args.is_empty() {
...
fn substitute_type_id(
    type_result: &TypeResult,
    type_id: pecan_analysis::types::TypeId,
    mapping: &HashMap<String, pecan_analysis::types::TypeId>,
) -> pecan_analysis::types::TypeId {
...
pub(crate) fn lower_function_with_name(
    ...
    generic_args: Option<HashMap<String, pecan_analysis::types::TypeId>>,
) -> CodegenResult<()> {
    let generic_args = generic_args.unwrap_or_default();
    let substitute = |type_id: pecan_analysis::types::TypeId| -> pecan_analysis::types::TypeId {
