...
pub fn str_new(ptr: *const u8, len: usize) -> *mut PecanStr {
    let size = std::mem::size_of::<PecanStr>();
    let allocation = alloc(size, std::ptr::null());
    if allocation.is_null() {
        panic!("string allocation failed");
    }
    let target = allocation.cast::<PecanStr>();
    unsafe {
        target.write(PecanStr { ptr, len });
    }
    target
}

pub fn str_len(value: *const PecanStr) -> usize {
    if value.is_null() {
...
pub fn array_new(_elem_size: usize, len: usize) -> *mut PecanArray {
    let size = std::mem::size_of::<PecanArray>();
    let allocation = alloc(size, std::ptr::null());
    if allocation.is_null() {
        panic!("array allocation failed");
    }
    let target = allocation.cast::<PecanArray>();
    unsafe {
        target.write(PecanArray {
...
pub fn sys_print(value: *const PecanStr) {
    if value.is_null() {
        panic!("null string handle");
    }
...
pub fn sys_println(value: *const PecanStr) {
    if value.is_null() {
        panic!("null string handle");
    }
    let (ptr, len) = unsafe { ((*value).ptr, (*value).len) };
...
pub fn panic_str(value: *const PecanStr) -> ! {
    if value.is_null() {
        panic!("null string handle");
    }
    let (ptr, len) = unsafe { ((*value).ptr, (*value).len) };
